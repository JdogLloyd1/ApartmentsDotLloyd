...
- Unit tests: parser round-trips all 19 buildings; loader inserts 19 rows then is a no-op on rerun.

**Verification**

1. `python -m scripts.extract_seed` produces a file with 19 entries matching the names in the static HTML.
2. `pytest tests/test_seed_loader.py` passes (includes idempotency test).
3. `sqlite3 alewife.db "select count(*), min(rating), max(rating) from building;"` reports `19, 2.9, 4.9`.
...
**Status (2026-04-20):** Complete. Live ORS refresh populates 19 walk + 19 drive travel times and 3 walk + 3 drive isochrone polygons. `/api/isochrones` ships real GeoJSON, `/api/buildings` prefers live walk/drive with seed fallback, and the frontend renders live polygons via `L.geoJSON`. 16 routing tests pass against recorded ORS fixtures.
...
**Status (2026-04-20):** Complete. `app/scrapers/` ships fully fixture-tested parsers (apartments.com + Google Places), a Playwright-based `HTMLFetcher`, idempotent `refresh_prices`/`refresh_ratings` services that write `price_snapshot` and `rating_snapshot` rows, an `app/seed/scrape_targets.json` sidecar so URLs/place_ids can be set without touching the auto-generated seed, and a top-level `python -m app.refresh_cli` that runs ORS + scrapers in one shot. `BuildingOut` now layers snapshot → travel-time → seed and surfaces `pricesFetchedAt` / `ratingFetchedAt` on every row. Verified end-to-end with a real Playwright run against a local fixture URL: snapshot landed in DB, score recomputed, payload reflects live values. 62 backend tests pass; ruff clean. Real apartments.com / Google Maps URLs in `scrape_targets.json` are still placeholders the user fills in for live production scraping.
...
### Sprint 6 — Refresh Orchestration & Caching ✅ COMPLETE
...
- TTL response cache (`app/cache.py`) wraps `/api/buildings` and `/api/isochrones`; both endpoints stamp `X-Data-Freshness` (seconds-since-cache-insertion). Verified live: first call freshness=0, second call 2s later freshness=2.
- `app/refresh_service.py` orchestrates ORS travel-times, isochrones, prices, and ratings; persists per-step status to `refresh_run`; invalidates the response cache on success.
- `POST /api/refresh` requires `Authorization: Bearer $REFRESH_BEARER_TOKEN`; returns `202` with `run_id`. `GET /api/refresh/{run_id}` polls status. Missing-token boots return `503`.
- APScheduler is wired into the FastAPI lifespan but **off by default** (`REFRESH_SCHEDULER_ENABLED=false`) — tests and one-shot CLI runs don't spawn cron jobs. Two jobs are registered when enabled: `daily_refresh` (full) and `hourly_prices` (price-only).
- Frontend header carries a freshness chip (turns yellow after 6h) and a hidden `Refresh now` button that becomes active when `window.ALEWIFE_REFRESH_TOKEN` is set.
- Tests added: `test_cache.py`, `test_freshness_header.py`, `test_refresh_service.py`, `test_refresh_endpoint.py`, `test_scheduler.py`. Full suite: **80 passed, 1 skipped**, ruff clean.
